...
export const AnonymousEntryForm: React.FC<Props> = ({ onStartMatching, onStartInstantDemo, stats }) => {
  ...
  return (
    <motion.div
      ...
      className="min-h-screen bg-indigo-600 p-2.5 sm:p-5 lg:p-6 font-sans flex flex-col justify-between overflow-x-hidden text-slate-900"
    >
      {/* Top Navbar */}
      <nav className="flex justify-between items-center mb-2 sm:mb-4 max-w-6xl mx-auto w-full">
        <NumaLogo size="md" />

        <div className="flex items-center gap-2 sm:gap-3">
          <div className="bg-indigo-500/30 text-indigo-100 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-xs font-bold backdrop-blur-md border border-indigo-400/20 flex items-center gap-1.5 sm:gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span>{stats.onlineUsers || 1} <span className="hidden sm:inline">Mahasiswa</span> Online</span>
          </div>
          <div className="hidden md:flex bg-indigo-500/30 text-indigo-100 px-3.5 py-1.5 rounded-full text-xs font-bold backdrop-blur-md border border-indigo-400/20 items-center gap-1.5 shadow-sm">
            ...
            <AnimatePresence mode="wait">
              <motion.span
                ...
                className="font-black text-amber-300 font-mono text-xs sm:text-sm"
              >
      ...
      <motion.div
        ...
        className="max-w-6xl mx-auto w-full bg-white rounded-3xl sm:rounded-[36px] lg:rounded-[44px] shadow-2xl flex flex-col lg:flex-row overflow-hidden border-4 sm:border-8 lg:border-[10px] border-indigo-400/30 my-auto"
      >
        {/* Left Side: Matchmaking Setup Form */}
        <div className="w-full lg:w-[500px] xl:w-[540px] p-3.5 sm:p-6 lg:p-7 flex flex-col justify-center">
          <div className="mb-2.5 sm:mb-3.5">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-[10px] sm:text-xs font-black uppercase tracking-wider mb-1">
              <Sparkles className="w-3 h-3 text-indigo-500" />
              <span>Virtual Matchmaking</span>
            </div>

            <h1 className="text-xl sm:text-3xl font-black leading-tight text-slate-900">
              Ready to focus <span className="text-indigo-600">together?</span>
            </h1>
          ...
          <form id="entry-form" onSubmit={handleSubmit} className="space-y-2.5 sm:space-y-3.5">
            ...
            {/* Trust Note & Instant Demo */}
            <div className="flex items-center justify-between text-[11px] sm:text-xs pt-2 px-1">
        ...
        <div className="flex-1 bg-indigo-50/60 p-3.5 sm:p-6 lg:p-7 flex flex-col justify-between relative border-t lg:border-t-0 lg:border-l border-indigo-100 gap-3 sm:gap-4">
          {/* Top Section: Action Button & Social Proof Stream */}
          <div className="flex flex-col items-center justify-center my-auto space-y-3 sm:space-y-4 w-full">
            {/* Primary Action Button (Moved to Right Side) */}
            <div className="relative w-full max-w-sm group">
              <div className="absolute -inset-1.5 bg-gradient-to-r from-indigo-600 via-indigo-500 to-amber-500 rounded-3xl blur-md opacity-60 group-hover:opacity-100 transition duration-300 animate-pulse"></div>
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                className="relative w-full py-3.5 sm:py-4.5 px-5 sm:px-6 rounded-2xl sm:rounded-3xl bg-indigo-600 hover:bg-indigo-700 text-white font-black shadow-xl shadow-indigo-300/40 transition-all cursor-pointer flex flex-col items-center justify-center gap-1 border border-indigo-400/30"
              >
                <div className="flex items-center justify-center gap-2 text-base sm:text-lg lg:text-xl">
                  <Users className="w-5 h-5 sm:w-6 sm:h-6 text-amber-300" />
                  <span>Cari Partner Sekarang</span>
                  <ArrowRight className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-200" />
                </div>
                <div className="flex items-center gap-1 text-[10px] sm:text-[11px] font-bold text-indigo-200">
                  <Lock className="w-3 h-3 text-emerald-400" />
                  <span>Peer-to-Peer • Encrypted & Anonymous</span>
                </div>
              </motion.button>
            </div>
            ...
            <div className="text-center w-full">
              <div className="flex -space-x-2.5 mb-1 justify-center">
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white bg-indigo-500 shadow-md"></div>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white bg-rose-500 shadow-md"></div>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white bg-amber-400 shadow-md"></div>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white bg-emerald-500 shadow-md flex items-center justify-center text-[9px] sm:text-[10px] font-black text-white">
                  +{stats.onlineUsers || 42}
                </div>
              </div>
              <p className="text-indigo-950 font-black text-[9px] sm:text-[10px] tracking-widest uppercase">
          ...
          <div className="w-full bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-xl border-2 border-indigo-100 space-y-2 mt-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
              <div className="flex items-center gap-1.5">
                <Trophy className="w-3.5 h-3.5 text-amber-500" />
                <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-indigo-950">Statistik Belajar Anda</span>
              </div>
              <div className="flex items-center gap-1 text-[11px] sm:text-xs font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
            ...
            <div className="grid grid-cols-2 gap-1.5 sm:gap-2 text-center">
              <div className="p-1.5 sm:p-2 rounded-xl sm:rounded-2xl bg-indigo-50/70 border border-indigo-100">
                <span className="text-[9px] sm:text-[10px] font-bold text-indigo-600 uppercase tracking-wider block">Waktu Fokus</span>
                <span className="text-sm sm:text-base font-black text-indigo-950 font-mono">{streakStats.totalMinutes} <span className="text-[9px] sm:text-[10px] font-normal text-slate-500">Menit</span></span>
              </div>
              <div className="p-1.5 sm:p-2 rounded-xl sm:rounded-2xl bg-emerald-50/70 border border-emerald-100">
                <span className="text-[9px] sm:text-[10px] font-bold text-emerald-700 uppercase tracking-wider block">Total Sesi</span>
                <span className="text-sm sm:text-base font-black text-emerald-950 font-mono">{streakStats.totalSessions} <span className="text-[9px] sm:text-[10px] font-normal text-slate-500">Sesi</span></span>
              </div>
            </div>
            ...
            {unlockedBadges.length > 0 && (
              <div className="pt-0.5">
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">Pencapaian Terbuka ({unlockedBadges.length}):</span>
                <div className="flex flex-wrap gap-1">
                  {unlockedBadges.map((badge) => (
                    <div
                      key={badge.id}
                      className="inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-lg sm:rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-900 text-[9px] sm:text-[10px] font-bold shadow-xs"
      ...
      {/* Footer */}
      <footer className="mt-2 sm:mt-3 flex flex-col sm:flex-row justify-between items-center text-indigo-200 px-4 text-[10px] sm:text-xs font-medium gap-1 max-w-6xl mx-auto w-full">
        <p className="text-center sm:text-left font-semibold">
          &copy; Numa Space • Privacy First • No History Stored
        </p>

        <div className="flex gap-3 sm:gap-6 font-bold uppercase tracking-widest text-[9px] sm:text-[11px]">
